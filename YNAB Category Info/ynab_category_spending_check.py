...
GROCERIES_CATEGORY_ID = config["DEFAULT"]["GroceriesCategoryID"]
...
def send_pushover_message(message, title):
    pushover = PushoverAPIClient(
        api_token=config["DEFAULT"]["PushoverToken"],
        user_key=config["DEFAULT"]["PushoverUserKey"],
    )
    pushover.send(title=title, message=message)
...
def main():
    ...
    if pushover_enabled and total_groceries_spent*-1 > THRESHOLD:
        ...
        try:
            send_pushover_message(message, "Grocery Spending Alert")
